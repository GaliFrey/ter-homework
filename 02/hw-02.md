...
4. Сгенерируйте новый или используйте свой текущий ssh-ключ. Запишите его открытую(public) часть в переменную **vms_ssh_public_root_key**.
    ...
    >   type        = string
    >   default     = "ssh-ed25519 AAAAC3N...u66PvcBdpP5"
...
5. Инициализируйте проект, выполните код. Исправьте намеренно допущенные синтаксические ошибки. Ищите внимательно, посимвольно. Ответьте, в чём заключается их суть.
    ...
    > Исправил ограничение версии
    > ![alt text](img/1/image-3.png)
    > #### Выполнение кода
    > Исправление блока `resource "yandex_compute_instance" "platform"`:
    > ```
    ...
    > }
    > ```
    > Основная проблема в опечатке в слове `standart` и в версии `4`, в YC есть только версия `standard-v4a`. Так как я использовал платформу `v3`, у меня минимальные значения `core_fraction` стали `20`, `CPU` минимум 2, а оперативную память можно было оставить `1`, требования по `0.5G` на каждое ядро. Это указано в [документации](https://yandex.cloud/ru/docs/compute/concepts/performance-levels)
...
7. Ответьте, как в процессе обучения могут пригодиться параметры ```preemptible = true``` и ```core_fraction=5``` в параметрах ВМ.
    > ```preemptible = true``` - прерываемая виртуальная машина, позволяет сэкономить деньги в процессе обучения, удешевляя стоимость ВМ. 
    > ```core_fraction=5``` - так же удешевляет стоимость ВМ
...
> Ошибка с подсетями, создал отдельную для зоны db
...
2. Создайте и используйте отдельную map(object) переменную для блока metadata, она должна быть общая для всех ваших ВМ.
   ...
   ```  
    ...
    >     default = {
    >       serial-port-enable = "1"
    >       ssh-keys           = "ssh-ed25519 AAAAC...BdpP5"
    >     }
    > }
    > ```
    > 
